<?php
 namespace WPAICG\WP_AI_Client; if (!defined('ABSPATH')) { exit; } class AIPKit_WP_AI_Client_Connectors { private static bool $syncing = false; public static function register_hooks(): void { add_action('admin_footer', [self::class, 'maybe_render_banner']); add_action('wp_ajax_aipkit_wp_ai_client_set_mode', [self::class, 'ajax_set_mode']); add_action('wp_connectors_init', [self::class, 'customize_registry'], 1000); add_action('updated_option', [self::class, 'maybe_bridge_connector_key'], 10, 3); add_action('added_option', [self::class, 'maybe_bridge_added_connector_key'], 10, 2); add_filter('script_module_data_options-connectors-wp-admin', [self::class, 'mark_managed_connectors_connected'], 1000); add_filter('wpai_has_ai_credentials', [self::class, 'declare_credentials_present'], 1000, 1); add_filter('wpai_pre_has_valid_credentials_check', [self::class, 'declare_credentials_valid'], 1000, 1); foreach (array_keys(AIPKit_WP_AI_Client_Settings::providers()) as $connector_id) { add_filter('wpai_is_' . $connector_id . '_connector_configured', [self::class, 'declare_connector_configured'], 1000, 2); } } public static function customize_registry(\WP_Connector_Registry $registry): void { if (!AIPKit_WP_AI_Client_Settings::is_effectively_managed()) { return; } foreach (AIPKit_WP_AI_Client_Settings::providers() as $connector_id => $config) { try { if ($registry->is_registered($connector_id)) { $registry->unregister($connector_id); } $auth = ['method' => 'none']; $registry->register($connector_id, [ 'name' => ($config['name'] ?? ucwords($connector_id)) . ' via AI Puffer', 'description' => $config['description'] ?? __('AI provider managed by AI Puffer.', 'gpt3-ai-content-generator'), 'type' => 'ai_provider', 'authentication' => $auth, 'plugin' => [ 'file' => plugin_basename(WPAICG_PLUGIN_DIR . 'gpt3-ai-content-generator.php'), 'is_active' => '__return_true', ], ]); } catch (\Throwable $e) { continue; } } } public static function maybe_render_banner(): void { if (!is_admin() || !\WPAICG\AIPKit_Role_Manager::user_can_manage_settings() || !self::is_connectors_screen()) { return; } if (!AIPKit_WP_AI_Client_Settings::is_supported()) { return; } $nonce = wp_create_nonce('aipkit_wp_ai_client_set_mode'); $managed = AIPKit_WP_AI_Client_Settings::is_effectively_managed(); if (!$managed && AIPKit_WP_AI_Client_Settings::is_banner_dismissed()) { return; } $configure_url = admin_url('admin.php?page=wpaicg&aipkit_module=settings'); $learn_more_url = apply_filters('aipkit_wp_ai_client_learn_more_url', 'https://docs.aipower.org/wordpress-ai-connectors'); $payload = [ 'nonce' => $nonce, 'ajaxUrl' => admin_url('admin-ajax.php'), 'settings' => $configure_url, 'learnUrl' => $learn_more_url, 'managed' => $managed, 'title' => $managed ? __('Managed by AI Puffer.', 'gpt3-ai-content-generator') : __('AI Puffer can manage your connectors.', 'gpt3-ai-content-generator'), 'sub' => $managed ? __('All WordPress AI requests run through AI Puffer.', 'gpt3-ai-content-generator') : __('Add usage logs, cost tracking, quotas, feature-level defaults, and more providers in one click.', 'gpt3-ai-content-generator'), 'enable' => __('Enable', 'gpt3-ai-content-generator'), 'dismiss' => __('Dismiss', 'gpt3-ai-content-generator'), 'configure' => __('Configure', 'gpt3-ai-content-generator'), 'stop' => __('Stop', 'gpt3-ai-content-generator'), 'learnMore' => __('Learn more', 'gpt3-ai-content-generator'), 'manageLead' => __('Need another provider? Add or edit providers in', 'gpt3-ai-content-generator'), 'manageLink' => __('AI Puffer settings', 'gpt3-ai-content-generator'), ]; ?>
        ...
        <?php
 } public static function ajax_set_mode(): void { check_ajax_referer('aipkit_wp_ai_client_set_mode', 'nonce'); if (!\WPAICG\AIPKit_Role_Manager::user_can_manage_settings()) { wp_send_json_error(['message' => 'forbidden'], 403); } $mode = isset($_POST['mode']) ? sanitize_key(wp_unslash($_POST['mode'])) : AIPKit_WP_AI_Client_Settings::MODE_OBSERVE; if ($mode === 'dismiss') { AIPKit_WP_AI_Client_Settings::set_banner_dismissed(true); wp_send_json_success(['dismissed' => true]); } AIPKit_WP_AI_Client_Settings::set_mode($mode); AIPKit_WP_AI_Client_Settings::set_banner_dismissed(false); wp_send_json_success(['mode' => AIPKit_WP_AI_Client_Settings::get_mode()]); } public static function maybe_bridge_connector_key(string $option, $old_value, $value): void { self::bridge_option_to_provider($option, $value); } public static function maybe_bridge_added_connector_key(string $option, $value): void { self::bridge_option_to_provider($option, $value); } public static function mark_managed_connectors_connected(array $data): array { if (!AIPKit_WP_AI_Client_Settings::is_effectively_managed()) { return $data; } if (empty($data['connectors']) || !is_array($data['connectors'])) { return $data; } foreach (array_keys(AIPKit_WP_AI_Client_Settings::providers()) as $connector_id) { if (empty($data['connectors'][$connector_id]['authentication'])) { continue; } if (!AIPKit_WP_AI_Client_Settings::provider_has_credentials($connector_id)) { continue; } $data['connectors'][$connector_id]['authentication']['isConnected'] = true; $data['connectors'][$connector_id]['authentication']['keySource'] = 'none'; } return $data; } public static function declare_credentials_present($has_credentials): bool { if (!AIPKit_WP_AI_Client_Settings::is_effectively_managed()) { return (bool) $has_credentials; } if ($has_credentials) { return true; } foreach (array_keys(AIPKit_WP_AI_Client_Settings::providers()) as $connector_id) { if (AIPKit_WP_AI_Client_Settings::provider_has_credentials($connector_id)) { return true; } } return false; } public static function declare_credentials_valid($valid) { if (!AIPKit_WP_AI_Client_Settings::is_effectively_managed()) { return $valid; } return true; } public static function declare_connector_configured($configured, array $connector_data = []): bool { if (!AIPKit_WP_AI_Client_Settings::is_effectively_managed()) { return (bool) $configured; } $hook_name = current_filter(); if (!is_string($hook_name)) { return (bool) $configured; } $prefix = 'wpai_is_'; $suffix = '_connector_configured'; if (substr($hook_name, 0, strlen($prefix)) !== $prefix || substr($hook_name, -strlen($suffix)) !== $suffix) { return (bool) $configured; } $connector_id = substr($hook_name, strlen($prefix), -strlen($suffix)); if ($connector_id === '') { return (bool) $configured; } return AIPKit_WP_AI_Client_Settings::provider_has_credentials($connector_id) ? true : (bool) $configured; } private static function bridge_option_to_provider(string $option, $value): void { if (self::$syncing || !AIPKit_WP_AI_Client_Settings::is_effectively_managed()) { return; } foreach (array_keys(AIPKit_WP_AI_Client_Settings::providers()) as $connector_id) { if ($option !== AIPKit_WP_AI_Client_Settings::connector_option_name($connector_id)) { continue; } self::$syncing = true; AIPKit_WP_AI_Client_Settings::sync_connector_key_to_provider($connector_id, is_scalar($value) ? (string) $value : ''); self::$syncing = false; return; } } private static function is_connectors_screen(): bool { global $pagenow; if ($pagenow === 'options-connectors.php') { return true; } $screen = function_exists('get_current_screen') ? get_current_screen() : null; if ($screen && isset($screen->id) && strpos((string) $screen->id, 'options-connectors') !== false) { return true; } return false; } } 