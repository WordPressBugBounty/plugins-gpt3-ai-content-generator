...
class AIPKit_WP_AI_Client_Connectors
{
    ...
    public static function register_hooks(): void
    {
        ...
        add_action('admin_init', [self::class, 'sync_provider_keys_to_connectors'], 1000);
        ...
        add_action('update_option_aipkit_options', [self::class, 'sync_provider_keys_to_connectors'], 1000, 0);
    ...
    public static function customize_registry(\WP_Connector_Registry $registry): void
    {
        ...
        foreach (AIPKit_WP_AI_Client_Settings::providers() as $connector_id => $config) {
            try {
                ...
                $auth = !empty($config['keyless'])
                    ? ['method' => 'none']
                    : [
                        'method' => 'api_key',
                        'setting_name' => AIPKit_WP_AI_Client_Settings::connector_option_name($connector_id),
                        'constant_name' => AIPKit_WP_AI_Client_Settings::connector_constant_name($connector_id),
                        'env_var_name' => AIPKit_WP_AI_Client_Settings::connector_constant_name($connector_id),
                    ];

                if (!empty($config['credentials_url']) && empty($config['keyless'])) {
                    $auth['credentials_url'] = $config['credentials_url'];
                }
    ...
    {
        if (!is_admin() || !current_user_can('manage_options') || !self::is_connectors_screen()) {
            return;
        }
    ...
    public static function ajax_set_mode(): void
    {
        ...
        if ($mode === AIPKit_WP_AI_Client_Settings::MODE_MANAGED) {
            AIPKit_WP_AI_Client_Settings::sync_provider_keys_to_connectors();
        }
    ...
    public static function mark_managed_connectors_connected(array $data): array
    {
        ...
        foreach (AIPKit_WP_AI_Client_Settings::providers() as $connector_id => $config) {
            ...
            if (empty($data['connectors'][$connector_id]['authentication']['keySource']) || $data['connectors'][$connector_id]['authentication']['keySource'] === 'none') {
                $data['connectors'][$connector_id]['authentication']['keySource'] = !empty($config['keyless']) ? 'none' : 'database';
            }
        }
