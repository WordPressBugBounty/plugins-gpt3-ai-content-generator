...
function start_stream_logic(
    ...
    try {
        ...
        $processorInstance->initialize_stream_state(
            $provider, $model, $conversation_uuid, ($base_log_data['bot_message_id'] ?? null), $base_log_data,
        ...
        if (!empty($curl_options_base['user-agent'])) curl_setopt($ch, CURLOPT_USERAGENT, $curl_options_base['user-agent']);
        ...
        if ($curl_error_num) {
            ...
            if ($triggers_addon_active && class_exists($trigger_manager_class) && class_exists($trigger_storage_class) && $log_storage_for_triggers) {
             ...
             $formatter->send_sse_error("API Error: {$api_error_message}", false); $processorInstance->set_error_occurred_status(true);
            ...
            if ($triggers_addon_active && class_exists($trigger_manager_class) && class_exists($trigger_storage_class) && $log_storage_for_triggers) {
             ...
             $formatter->send_sse_error($no_data_error_msg, false); $processorInstance->set_error_occurred_status(true);
            ...
            if ($processorInstance->get_grounding_metadata() !== null) $done_data['grounding_metadata'] = $processorInstance->get_grounding_metadata();
