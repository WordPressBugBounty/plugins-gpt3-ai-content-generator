 ...
 */
function get_image_models_logic(OpenRouterProviderStrategy $strategyInstance, array $api_params): array|WP_Error {
    $url = $strategyInstance->build_api_url('models', $api_params);
    if (is_wp_error($url)) {
        return $url;
    }

    $url .= (str_contains($url, '?') ? '&' : '?') . 'output_modalities=image';
