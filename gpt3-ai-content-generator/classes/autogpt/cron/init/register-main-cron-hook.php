...
/**
 * Schedules or unschedules the main queue processing cron hook based on active task status.
 *
 ...
 * @return void
 */
function register_main_cron_hook_logic(string $main_cron_hook, int $active_task_count, bool $did_active_tasks_exist): void
{
    $option_key_tasks_exist = 'aipkit_active_tasks_exist';

    if ($active_task_count === 0) {
        // Only clear the cron if tasks existed before but now there are none.
