...
class AIPKit_Update_Automated_Task_Status_Action extends AIPKit_Automated_Task_Base_Ajax_Action {
    ...
    public function handle_request() {
        ...
        if (empty($task_id) || $status === null) { $this->send_wp_error(new WP_Error('missing_params_status_update', __('Task ID and status are required.', 'gpt3-ai-content-generator')), 400); return; }

        $task = $wpdb->get_row($wpdb->prepare("SELECT task_config FROM {$this->tasks_table_name} WHERE id = %d", $task_id), ARRAY_A);
        if (!$task) { $this->send_wp_error(new WP_Error('task_not_found_status', __('Task not found.', 'gpt3-ai-content-generator')), 404); return; }
