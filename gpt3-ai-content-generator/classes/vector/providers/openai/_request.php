...
function _request_logic(AIPKit_Vector_OpenAI_Strategy $strategyInstance, string $method, string $url, array $body = [], bool $is_file_upload = false): array|WP_Error {
    ...
    if ($is_file_upload) {
        ...
        $curl_headers = [];
        foreach ($headers as $key => $value) { $curl_headers[] = "{$key}: {$value}"; }
    ...
    if ($status_code >= 400) {
        ...
        return new WP_Error('openai_vector_api_error', sprintf(__('OpenAI Vector API Error (%d): %s', 'gpt3-ai-content-generator'), $status_code, $error_msg));
    }
