<?php
// File: /Applications/MAMP/htdocs/wordpress/wp-content/plugins/gpt3-ai-content-generator/classes/vector/providers/qdrant/ensure-payload-indexes.php
...
function normalize_filter_payload_keys_logic(array $filter): array {
    if (isset($filter['key']) && is_string($filter['key'])) {
        if (str_starts_with($filter['key'], 'payload.')) {
            $filter['key'] = substr($filter['key'], 8);
        }
 ...
 */
function ensure_payload_indexes_for_filter_logic(AIPKit_Vector_Qdrant_Strategy $strategyInstance, string $collection_name, array $filter): bool|WP_Error {
    ...
    foreach ($conditions as $condition) {
        ...
        if (is_wp_error($response)) {
            ...
            if (str_contains($message, 'already exists') || str_contains($message, 'already has')) {
