<?php
 namespace WPAICG\Shortcodes\TokenUsage\Render; require_once __DIR__ . '/render_module_table_header.php'; if (!defined('ABSPATH')) { exit; } function render_dashboard_logic( \WPAICG\Shortcodes\AIPKit_Token_Usage_Shortcode $facade, array $usage_data, bool $show_chatbot = true, bool $show_aiforms = true, bool $show_imagegenerator = true, string $dashboard_title = '', string $dashboard_intro = '', bool $show_buy_credits = true, string $buy_credits_label = '', string $buy_credits_url = '', bool $show_purchase_history = true ): string { ob_start(); $shop_page_url = trim($buy_credits_url); if ($shop_page_url === '') { $shop_page_url = get_option('aipkit_token_shop_page_url', ''); } if ($shop_page_url === '' && function_exists('wc_get_page_id')) { $shop_page_url = get_permalink(wc_get_page_id('shop')); } $dashboard_title = trim($dashboard_title) !== '' ? $dashboard_title : __('Credits & Usage', 'gpt3-ai-content-generator'); $dashboard_intro = trim($dashboard_intro) !== '' ? $dashboard_intro : __('View your credits, purchases, and quotas.', 'gpt3-ai-content-generator'); $buy_credits_label = trim($buy_credits_label) !== '' ? $buy_credits_label : __('Buy credits', 'gpt3-ai-content-generator'); ?>
