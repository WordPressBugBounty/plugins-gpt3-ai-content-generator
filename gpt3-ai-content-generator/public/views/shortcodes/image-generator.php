<?php
 if (!defined('ABSPATH')) { exit; } use WPAICG\AIPKit_Providers; $openai_models_display = []; $google_models_display = [ 'image' => [], 'video' => [] ]; $openrouter_models_display = []; $xai_models_display = []; $replicate_models_display = []; $azure_models_display = []; if (class_exists('\\WPAICG\\AIPKit_Providers')) { $openai_models = AIPKit_Providers::get_openai_image_models(); if (!empty($openai_models)) { foreach ($openai_models as $mdl) { $mid = is_array($mdl) ? ($mdl['id'] ?? null) : (is_string($mdl) ? $mdl : null); $mname = is_array($mdl) ? ($mdl['name'] ?? $mid) : $mid; if ($mid) { $openai_models_display[$mid] = $mname; } } } $google_image_models = AIPKit_Providers::get_google_image_models(); if (!empty($google_image_models)) { foreach ($google_image_models as $mdl) { $mid = is_array($mdl) ? ($mdl['id'] ?? null) : (is_string($mdl) ? $mdl : null); $mname = is_array($mdl) ? ($mdl['name'] ?? $mid) : $mid; if ($mid) { $google_models_display['image'][$mid] = $mname; } } } $google_video_models = AIPKit_Providers::get_google_video_models(); if (!empty($google_video_models)) { foreach ($google_video_models as $mdl) { $mid = is_array($mdl) ? ($mdl['id'] ?? null) : (is_string($mdl) ? $mdl : null); $mname = is_array($mdl) ? ($mdl['name'] ?? $mid) : $mid; if ($mid) { $google_models_display['video'][$mid] = $mname; } } } $openrouter_models = AIPKit_Providers::get_openrouter_image_models(); if (!empty($openrouter_models)) { foreach ($openrouter_models as $mdl) { $mid = is_array($mdl) ? ($mdl['id'] ?? null) : (is_string($mdl) ? $mdl : null); $mname = is_array($mdl) ? ($mdl['name'] ?? $mid) : $mid; if ($mid) { $openrouter_models_display[$mid] = $mname; } } } $xai_models = AIPKit_Providers::get_xai_image_models(); if (!empty($xai_models)) { foreach ($xai_models as $mdl) { $mid = is_array($mdl) ? ($mdl['id'] ?? null) : (is_string($mdl) ? $mdl : null); $mname = is_array($mdl) ? ($mdl['name'] ?? $mid) : $mid; if ($mid) { $xai_models_display[$mid] = $mname; } } } $replicate_models = AIPKit_Providers::get_replicate_models(); if (!empty($replicate_models)) { foreach ($replicate_models as $mdl) { $mid = is_array($mdl) ? ($mdl['id'] ?? null) : (is_string($mdl) ? $mdl : null); $mname = is_array($mdl) ? ($mdl['name'] ?? $mid) : $mid; if ($mid) { $replicate_models_display[$mid] = $mname; } } } $azure_models_list = AIPKit_Providers::get_azure_image_models(); if (!empty($azure_models_list)) { foreach ($azure_models_list as $mdl) { $mid = is_array($mdl) ? ($mdl['id'] ?? null) : (is_string($mdl) ? $mdl : null); $mname = is_array($mdl) ? ($mdl['name'] ?? $mid) : $mid; if ($mid) { $azure_models_display[$mid] = $mname; } } } } $theme_class = 'aipkit-theme-' . esc_attr($theme); $allowed_modes = ['generate', 'edit', 'both']; $shortcode_mode = isset($mode) ? sanitize_key((string) $mode) : 'generate'; if (!in_array($shortcode_mode, $allowed_modes, true)) { $shortcode_mode = 'generate'; } $allowed_default_modes = ['generate', 'edit']; $shortcode_default_mode = isset($default_mode) ? sanitize_key((string) $default_mode) : 'generate'; if (!in_array($shortcode_default_mode, $allowed_default_modes, true)) { $shortcode_default_mode = 'generate'; } $allow_mode_switch = ($shortcode_mode === 'both' && !empty($show_mode_switch)); $current_image_mode = $shortcode_mode === 'both' ? $shortcode_default_mode : $shortcode_mode; if (!in_array($current_image_mode, $allowed_default_modes, true)) { $current_image_mode = 'generate'; } $ui_text_settings = isset($ui_text) && is_array($ui_text) ? $ui_text : []; $get_ui_text = static function (string $key, string $default) use ($ui_text_settings): string { if (!isset($ui_text_settings[$key])) { return $default; } $value = sanitize_text_field((string) $ui_text_settings[$key]); return $value !== '' ? $value : $default; }; $generate_label = $get_ui_text('generate_label', __('Generate', 'gpt3-ai-content-generator')); $edit_label = $get_ui_text('edit_label', __('Edit Image', 'gpt3-ai-content-generator')); $mode_generate_label = $get_ui_text('mode_generate_label', __('Generate', 'gpt3-ai-content-generator')); $mode_edit_label = $get_ui_text('mode_edit_label', __('Edit', 'gpt3-ai-content-generator')); $generate_placeholder = $get_ui_text('generate_placeholder', __('Describe the image you want to generate...', 'gpt3-ai-content-generator')); $edit_placeholder = $get_ui_text('edit_placeholder', __('Describe how you want to edit the uploaded image...', 'gpt3-ai-content-generator')); $source_image_label = $get_ui_text('source_image_label', __('Source image', 'gpt3-ai-content-generator')); $upload_dropzone_title = $get_ui_text('upload_dropzone_title', __('Drop image here or click to upload', 'gpt3-ai-content-generator')); $upload_dropzone_meta = $get_ui_text('upload_dropzone_meta', __('JPG, PNG, WEBP, GIF up to 10MB', 'gpt3-ai-content-generator')); $upload_hint = $get_ui_text('upload_hint', __('Upload an image (JPG, PNG, WEBP, GIF up to 10MB), then describe the edits in the prompt.', 'gpt3-ai-content-generator')); $xai_upload_dropzone_meta = __('JPG or PNG up to 10MB', 'gpt3-ai-content-generator'); $xai_upload_hint = __('xAI image editing supports JPG and PNG source images only.', 'gpt3-ai-content-generator'); $history_title = $get_ui_text('history_title', __('Your Images', 'gpt3-ai-content-generator')); $initial_prompt_placeholder = $current_image_mode === 'edit' ? $edit_placeholder : $generate_placeholder; $initial_action_label = $current_image_mode === 'edit' ? $edit_label : $generate_label; ?>
<div
                    ...
                    <?php if ($show_provider) : ?>
                        <div class="aipkit_form-group aipkit_image_gen_option">
                            ...
                            <select id="aipkit_public_image_provider" name="image_provider" class="aipkit_form-input" data-aipkit-provider-notice-target="aipkit_provider_notice_image_generator">
                                <?php
 $all_providers = ['OpenAI', 'Google', 'OpenRouter', 'xAI', 'Azure', 'Replicate']; $allowed_models_arr = !empty($allowed_models) ? array_map('trim', explode(',', strtolower($allowed_models))) : []; if (!empty($allowed_models_arr)) { $derived = []; $openai_lookup = array_flip(array_map('strtolower', array_keys($openai_models_display))); $google_lookup = []; foreach ($google_models_display as $type => $models_array) { foreach ($models_array as $id => $name) { $google_lookup[strtolower((string) $id)] = true; } } $openrouter_lookup = array_flip(array_map('strtolower', array_keys($openrouter_models_display))); $xai_lookup = array_flip(array_map('strtolower', array_keys($xai_models_display))); $azure_lookup = array_flip(array_map('strtolower', array_keys($azure_models_display))); $replicate_lookup = array_flip(array_map('strtolower', array_keys($replicate_models_display))); foreach ($allowed_models_arr as $mid) { if (isset($openai_lookup[$mid])) { $derived['OpenAI'] = true; } elseif (isset($google_lookup[$mid])) { $derived['Google'] = true; } elseif (isset($openrouter_lookup[$mid])) { $derived['OpenRouter'] = true; } elseif (isset($xai_lookup[$mid])) { $derived['xAI'] = true; } elseif (isset($azure_lookup[$mid])) { $derived['Azure'] = true; } elseif (isset($replicate_lookup[$mid])) { $derived['Replicate'] = true; } elseif (strpos($mid, '/') !== false) { if (strpos($mid, ':') !== false) { $derived['Replicate'] = true; } else { $derived['OpenRouter'] = true; } } } $providers_to_show = array_values(array_intersect($all_providers, array_keys($derived))); if (empty($providers_to_show)) { $providers_to_show = $all_providers; } } else { $providers_to_show = $all_providers; } foreach ($providers_to_show as $provider_name) : ?>
                        ...
                        <div class="aipkit_form-group aipkit_image_gen_option">
                            ...
                            <select id="aipkit_public_image_model" name="image_model" class="aipkit_form-input">
                                 ...
                                 <?php if ($final_provider === 'OpenAI'): ?>
                                    <?php
 $sorted_openai_keys_render = ['gpt-image-2', 'gpt-image-1.5', 'gpt-image-1', 'gpt-image-1-mini']; $final_openai_models_render = []; foreach ($sorted_openai_keys_render as $key) { if (isset($openai_models_display[$key])) { $final_openai_models_render[$key] = $openai_models_display[$key]; } } foreach ($openai_models_display as $id => $name) { if (!isset($final_openai_models_render[$id])) { $final_openai_models_render[$id] = $name; } } ?>
                                 ...
                                 <?php elseif ($final_provider === 'Google'): ?>
                                     ...
                                     <?php endforeach; ?>
                                     <?php  $model_found = false; foreach ($google_models_display as $type => $models_array) { if (array_key_exists($final_model, $models_array)) { $model_found = true; break; } } if ($final_model && !$model_found) : ?>
                                        <option value="<?php echo esc_attr($final_model); ?>" selected><?php echo esc_html($final_model); ?> (Manual)</option>
                                     <?php endif; ?>
                                 ...
                                 <?php elseif ($final_provider === 'Azure'): ?>
                                     <?php  $azure_models_display_rows = AIPKit_Providers::get_azure_image_models(); ?>
                                     ...
                                     <?php endforeach; ?>
                                     <?php  $model_found = false; foreach ($azure_models_display_rows as $model) { if ($model['id'] === $final_model) { $model_found = true; break; } } if ($final_model && !$model_found) : ?>
                                        <option value="<?php echo esc_attr($final_model); ?>" selected><?php echo esc_html($final_model); ?> (Manual)</option>
                                     <?php endif; ?>
                                 <?php else: ?>
