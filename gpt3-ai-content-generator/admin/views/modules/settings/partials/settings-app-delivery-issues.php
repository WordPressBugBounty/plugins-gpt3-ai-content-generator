<?php
 if (!defined('ABSPATH')) { exit; } $is_pro_plan = class_exists('\WPAICG\\aipkit_dashboard') && \WPAICG\aipkit_dashboard::is_pro_plan(); $log_store_class = '\WPAICG\\Lib\\Integrations\\Logs\\AIPKit_Recipe_Delivery_Log_Store'; $recipe_store_class = '\WPAICG\\Lib\\Integrations\\Recipes\\AIPKit_Stored_Recipes'; if ( !$is_pro_plan || !class_exists($log_store_class) || !class_exists($recipe_store_class) || !method_exists($log_store_class, 'get_recent_failed_attempts') ) { return; } $delivery_issues = $log_store_class::get_recent_failed_attempts(5); if (empty($delivery_issues)) { return; } ?>
        ...
        <div class="aipkit_settings_app_delivery_issues_main">
            ...
            <div class="aipkit_settings_app_delivery_issue_list" data-aipkit-app-delivery-issue-list>
                <?php foreach ($delivery_issues as $issue) : ?>
                    <?php
 $issue_uuid = sanitize_text_field((string) ($issue['attempt_uuid'] ?? '')); $request_summary = isset($issue['request_summary']) && is_array($issue['request_summary']) ? $issue['request_summary'] : []; $response_summary = isset($issue['response_summary']) && is_array($issue['response_summary']) ? $issue['response_summary'] : []; $issue_recipe = method_exists($recipe_store_class, 'get_recipe_by_id') ? $recipe_store_class::get_recipe_by_id((string) ($issue['recipe_id'] ?? '')) : null; $issue_type = sanitize_key((string) ($response_summary['status'] ?? 'error')); $issue_label = $issue_type === 'reauth_required' ? __('Reauth Required', 'gpt3-ai-content-generator') : __('Failed', 'gpt3-ai-content-generator'); $issue_message = sanitize_text_field((string) (($response_summary['message'] ?? '') ?: ($issue['error_message'] ?? ''))); $issue_recipe_name = sanitize_text_field((string) (($request_summary['recipe_name'] ?? '') ?: ($issue_recipe['name'] ?? __('Recipe', 'gpt3-ai-content-generator')))); $issue_connection_name = sanitize_text_field((string) ($request_summary['connection_name'] ?? '')); $is_replayable = $issue_uuid !== '' && !empty($issue['event_snapshot']) && is_array($issue_recipe); ?>
