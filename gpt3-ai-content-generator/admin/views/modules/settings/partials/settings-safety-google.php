<?php
 if (!defined('ABSPATH')) exit; use WPAICG\Core\Providers\Google\GoogleSettingsHandler; $safety_settings = []; $category_thresholds = []; if (class_exists(GoogleSettingsHandler::class)) { $safety_settings = GoogleSettingsHandler::get_safety_settings(); foreach ($safety_settings as $setting) { if (isset($setting['category'], $setting['threshold'])) { $category_thresholds[$setting['category']] = $setting['threshold']; } } } else { return; } $safety_thresholds_map = array( 'BLOCK_NONE' => 'Block None', 'BLOCK_LOW_AND_ABOVE' => 'Block Few', 'BLOCK_MEDIUM_AND_ABOVE' => 'Block Some', 'BLOCK_ONLY_HIGH' => 'Block Most', ); $safety_categories_ordered = [ 'HARM_CATEGORY_HARASSMENT' => __('Harassment', 'gpt3-ai-content-generator'), 'HARM_CATEGORY_HATE_SPEECH' => __('Hate Speech', 'gpt3-ai-content-generator'), 'HARM_CATEGORY_SEXUALLY_EXPLICIT' => __('Sexually Explicit', 'gpt3-ai-content-generator'), 'HARM_CATEGORY_DANGEROUS_CONTENT' => __('Dangerous Content', 'gpt3-ai-content-generator'), 'HARM_CATEGORY_CIVIC_INTEGRITY' => __('Civic Integrity', 'gpt3-ai-content-generator'), ]; ?>
<div
...
>
    <?php foreach ($safety_categories_ordered as $category_key => $category_label): ?>
        <?php
 $current_threshold = $category_thresholds[$category_key] ?? 'BLOCK_NONE'; $input_name_short = strtolower(str_replace('HARM_CATEGORY_', '', $category_key)); $input_id = 'aipkit_safety_' . $input_name_short; ?>
