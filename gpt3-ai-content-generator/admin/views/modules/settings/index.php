<?php
 use WPAICG\AIPKIT_AI_Settings; use WPAICG\AIPKit_Providers; use WPAICG\aipkit_dashboard; use WPAICG\Core\Providers\Google\GoogleSettingsHandler; use WPAICG\Images\AIPKit_Image_Settings_Ajax_Handler; if (!defined('ABSPATH')) { exit; } $google_settings_handler_path = WPAICG_PLUGIN_DIR . 'classes/core/providers/google/bootstrap-provider-strategy.php'; if (!class_exists(GoogleSettingsHandler::class) && file_exists($google_settings_handler_path)) { require_once $google_settings_handler_path; } elseif (!class_exists(GoogleSettingsHandler::class)) { echo '<div class="notice notice-error"><p>Error: Google Settings component failed to load. Safety settings cannot be displayed.</p></div>'; } $aipkit_options = get_option('aipkit_options', array()); $aipkit_options = is_array($aipkit_options) ? $aipkit_options : []; AIPKit_Providers::get_all_providers(); $ai_params = AIPKIT_AI_Settings::get_ai_parameters(); $all_api_keys = AIPKIT_AI_Settings::get_api_keys(); $public_api_key = $all_api_keys['public_api_key'] ?? ''; $is_pro = class_exists('\WPAICG\aipkit_dashboard') && aipkit_dashboard::is_pro_plan(); $safety_settings = class_exists(GoogleSettingsHandler::class) ? GoogleSettingsHandler::get_safety_settings() : []; $category_thresholds = array(); if (is_array($safety_settings)) { foreach ($safety_settings as $setting) { if (isset($setting['category'], $setting['threshold'])) { $category_thresholds[$setting['category']] = $setting['threshold']; } } } $current_provider = AIPKit_Providers::get_current_provider(); $openai_data = AIPKit_Providers::get_provider_data('OpenAI'); $openrouter_data = AIPKit_Providers::get_provider_data('OpenRouter'); $google_data = AIPKit_Providers::get_provider_data('Google'); $azure_data = AIPKit_Providers::get_provider_data('Azure'); $claude_data = AIPKit_Providers::get_provider_data('Claude'); $deepseek_data = AIPKit_Providers::get_provider_data('DeepSeek'); $xai_data = AIPKit_Providers::get_provider_data('xAI'); $ollama_data = AIPKit_Providers::get_provider_data('Ollama'); $elevenlabs_data = AIPKit_Providers::get_provider_data('ElevenLabs'); $replicate_data = AIPKit_Providers::get_provider_data('Replicate'); $pexels_data = AIPKit_Providers::get_provider_data('Pexels'); $pixabay_data = AIPKit_Providers::get_provider_data('Pixabay'); $pinecone_data = AIPKit_Providers::get_provider_data('Pinecone'); $qdrant_data = AIPKit_Providers::get_provider_data('Qdrant'); $chroma_data = AIPKit_Providers::get_provider_data('Chroma'); $elevenlabs_voice_list = AIPKit_Providers::get_elevenlabs_voices(); $elevenlabs_model_list = AIPKit_Providers::get_elevenlabs_models(); $replicate_model_list = AIPKit_Providers::get_replicate_models(); $pinecone_index_list = AIPKit_Providers::get_pinecone_indexes(); $qdrant_collection_list = AIPKit_Providers::get_qdrant_collections(); $chroma_collection_list = AIPKit_Providers::get_chroma_collections(); $image_generator_settings = class_exists(AIPKit_Image_Settings_Ajax_Handler::class) ? AIPKit_Image_Settings_Ajax_Handler::get_settings() : []; $replicate_image_settings = isset($image_generator_settings['replicate']) && is_array($image_generator_settings['replicate']) ? $image_generator_settings['replicate'] : []; $replicate_disable_safety_checker = array_key_exists('disable_safety_checker', $replicate_image_settings) ? (bool) $replicate_image_settings['disable_safety_checker'] : true; $temperature = $ai_params['temperature']; $top_p = $ai_params['top_p']; $openai_store_conversation = isset($openai_data['store_conversation']) ? $openai_data['store_conversation'] : '0'; $safety_thresholds = array( 'BLOCK_NONE' => 'Block None', 'BLOCK_LOW_AND_ABOVE' => 'Block Few', 'BLOCK_MEDIUM_AND_ABOVE' => 'Block Some', 'BLOCK_ONLY_HIGH' => 'Block Most', ); $openai_defaults = AIPKit_Providers::get_provider_defaults('OpenAI'); $openrouter_defaults = AIPKit_Providers::get_provider_defaults('OpenRouter'); $google_defaults = AIPKit_Providers::get_provider_defaults('Google'); $azure_defaults = AIPKit_Providers::get_provider_defaults('Azure'); $claude_defaults = AIPKit_Providers::get_provider_defaults('Claude'); $deepseek_defaults = AIPKit_Providers::get_provider_defaults('DeepSeek'); $xai_defaults = AIPKit_Providers::get_provider_defaults('xAI'); $ollama_defaults = AIPKit_Providers::get_provider_defaults('Ollama'); $pinecone_defaults = AIPKit_Providers::get_provider_defaults('Pinecone'); $qdrant_defaults = AIPKit_Providers::get_provider_defaults('Qdrant'); $chroma_defaults = AIPKit_Providers::get_provider_defaults('Chroma'); $providers = ['OpenAI', 'Google', 'Claude', 'OpenRouter', 'Azure', 'Ollama', 'DeepSeek', 'xAI']; $provider_select_options = class_exists('\\WPAICG\\AIPKit_Provider_Model_List_Builder') ? \WPAICG\AIPKit_Provider_Model_List_Builder::get_provider_options($providers, $is_pro) : []; ?>
...
<div id="aipkit_google_tts_voices_json_main" style="display:none;" data-voices="<?php
 $google_voices_main = class_exists(GoogleSettingsHandler::class) ? GoogleSettingsHandler::get_synced_google_tts_voices() : []; echo esc_attr(wp_json_encode($google_voices_main ?: [])); ?>"></div>
